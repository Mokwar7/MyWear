...
function Card({ isLogged }) {
    ...
    function onLoad() {
        fetch(`http://localhost:3000/catalog/${location.pathname.split('/')[2]}`, {
            ...
            .then((res) => {
                ...
                fetch(`http://localhost:3000/cart/${res.data[0].article}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        "Authorization": localStorage.getItem('jwt') ? localStorage.getItem('jwt') : ''
    ...
    function handleClickSize(e) {
        ...
        alreadyIncart.forEach((item) => {
            if (e.target.closest('.card__label').querySelector('.card__size').textContent.split(' EU')[0] == item.size) {
                console.log('s')
                                    ...
                                    sizesNPrices.map((item, i) => {
                                        return (
                                            <label htmlFor="" className="card__label" onClick={handleClickSize} key={i} >
                                                <input type="radio" name="size" index={i} />
                    ...
                    </div>
                    {
                        inCart == 0
                            ...
                            <div className="card__buy card__cart">
                                <button className="card__cart_toggle" onClick={removeFromCart}>–</button>
                                {inCart} в корзине
