...
function Header({ isLogged, getCart }) {
  ...
  function handleRemove(e) {
    if (e.target.closest('.cart__count-cont').querySelector('.cart__brand').textContent == 1) {
      ...
      setCart(cart.filter((item) => {
        if (item.article != e.target.getAttribute('article')) {
          return true
        } else {
          if (item.size != e.target.getAttribute('size')) {
            return true
          } return false
        }
      }))
        ...
        <div className={"cart" + (isCart == true ? ' cart_active' : '')}>
          ...
          }
          {
            cart != undefined ?
              cart.map((item, i) => {
                return (
                  <NavLink to={`/catalog/${item.article}`} className="cart__card" key={i} onClick={handleClickCart}>
